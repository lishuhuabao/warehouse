...
#define TX_OUTPUT_POWER (dataSaveS.txPwr) // 定义发送功率  低功耗
...
void OnRxDone(uint8_t *payload, uint16_t size, int16_t _rssi, int8_t _snr)
{
	...
	if (((COMM_LORA_STEP_UPDATA == commLoraCtrlS.step) && (0xFF != RxTxBuffer[2])) || ((COMM_LORA_STEP_PC_COMM == commLoraCtrlS.step) && (0xFF == RxTxBuffer[2])))
	{
		loraRecFlag = 0;
		SfTimerStart(STIMER_SLEEP, SYSTEM_WAIT_SLEEP_TIME, 1);
	}
...
{
	SX1276Init();
	// adxs=BspLoraGetFreqByCh(dataSaveS.ch);
	// SX1276SetFreq(BspLoraGetFreqByCh(dataSaveS.ch));
	TRACE("Lora netch:%d", dataSaveS.ch);
