...
static void __TaskSendTraceCb(void *args);
...
static taskSt taskList[] = {
	...
	{TASK_SEND_TRACE, ALWAYS_ALIVE, NULL, __TaskSendTraceCb},
};
...
static void __TaskSendTraceCb(void *args)
{
	...
	if ((curTick < lastTick) || (curTick - lastTick > 3000)) // 3s task
	{
		lastTick = curTick;
		TRACE("MCU Running, tick:%d", lastTick);
	}
}
